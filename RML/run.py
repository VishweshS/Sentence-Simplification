...
def init_config():
    ...
    parser.add_argument('--log_file', default=None, type=str, help='save log training statistics to a file')
    ...
    parser.add_argument('--input_sent', default=None, type=str, help='input sentence for testing in interactive mode')
...
class NMT(nn.Module):
    ...
    def translate(self, src_sents, beam_size=None, to_word=True):
        ...
        while len(completed_hypotheses) < beam_size and t < args.decode_max_time_step:
            ...
            live_hyp_ids = []
            new_hyp_scores = []
            for prev_hyp_id, word_id, new_hyp_score in list(zip(prev_hyp_ids.cpu().data, word_ids.cpu().data, top_new_hyp_scores.cpu().data)):
        ...
        ranked_hypotheses = sorted(list(zip(completed_hypotheses, completed_hypothesis_scores)), key=lambda x: x[1], reverse=True)
    ...
    while True:
        ...
        for src_sents, tgt_sents in data_iter(train_data, batch_size=args.batch_size):
            ...
            if train_iter % args.log_every == 0:
                ...
                logs_mle.write('epoch %d, iter %d, avg. loss %.2f, avg. ppl %.2f ' \
            ...
            if train_iter % args.valid_niter == 0:
                ...
                logs_mle.write('epoch %d, iter %d, cum. loss %.2f, cum. ppl %.2f cum. examples %d\n\n' % (epoch, train_iter,
                                                                                         cum_loss / cum_batches,
                                                                                         np.exp(cum_loss / cum_tgt_words),
                                                                                         cum_examples))
                ...
                logs_mle.write('begin validation ...\n')
                model.eval()
                ...
                if args.valid_metric in ['bleu', 'word_acc', 'sent_acc']:
                    ...
                    print('validation: iter %d, dev. ppl %f, dev. %s %f' % (train_iter, dev_ppl, args.valid_metric, valid_metric),
                          file=sys.stderr)
                    logs_mle.write('validation: iter %d, dev. ppl %f, dev. %s %f\n\n' % (train_iter, dev_ppl, args.valid_metric, valid_metric))
                else:
                    valid_metric = -dev_ppl
                    print('validation: iter %d, dev. ppl %f' % (train_iter, dev_ppl),
                          file=sys.stderr)
                    logs_mle.write('validation: iter %d, dev. ppl %f\n\n' % (train_iter, dev_ppl))
                ...
                if valid_num > args.save_model_after:
                    ...
                    logs_mle.write('save model to [%s]\n\n' % model_file)
                    model.save(model_file)

                if (not is_better_than_last) and args.lr_decay:
                    lr = optimizer.param_groups[0]['lr'] * args.lr_decay
                    print('decay learning rate to %f' % lr, file=sys.stderr)
                    logs_mle.write('decay learning rate to %f\n\n' % lr)
                ...
                if is_better:
                    ...
                    if valid_num > args.save_model_after:
                        ...
                        logs_mle.write('save currently the best model ..\n\n')
                    ...
                    logs_mle.write('hit patience %d\n\n' % patience)
                    if patience == args.patience:
                        print('early stop!', file=sys.stderr)
                        logs_mle.write('early stop!\n')
                        print('the best model is from iteration [%d]' % best_model_iter, file=sys.stderr)
                        logs_mle.write('the best model is from iteration [%d]\n\n' % best_model_iter)
                        exit(0)
...
def train_raml(args):
    ...
    if args.raml_sample_mode == 'pre_sample':
        ...
        print('read in raml training data...\n', file=sys.stderr, end='')
    ...
    while True:
        ...
        for src_sents, tgt_sents in data_iter(train_data, batch_size=args.batch_size):
            ...
            if args.raml_sample_mode == 'pre_sample':
                ...
                for src_sent, tgt_sent in list(zip(src_sents, tgt_sents)):
                    ...
                    for i, e in enumerate(e_samples):
                        if e > 0:
                            ...
                            for pos, word in list(zip(old_word_pos, new_words)):
                    ...
                    if args.raml_sample_mode == 'hamming_distance_impt_sample':
                        tgt_sample_weights = [math.exp(bleu_score / tau) / math.exp(-e / tau) for e, bleu_score in list(zip(e_samples, bleu_scores))]
                    ...
                    if args.debug:
                        ...
                        for tgt_sample, e, bleu_score, weight in list(zip(tgt_samples, e_samples, bleu_scores,
                                                                     tgt_sample_weights)):
            ...
            logs_raml = open(args.log_file, 'a')
            ...
            if train_iter % args.log_every == 0:
                print('epoch %d, iter %d, avg. loss %.2f, '
                      ...
                      'speed %.2f words/sec, time elapsed %.2f sec' % (epoch, train_iter,
                                                                       ...
                                                                       time.time() - begin_time),
                      file=sys.stderr)
                logs_raml.write('epoch %d, iter %d, avg. loss %.2f, '
                      'avg. ppl %.2f cum. examples %d, '
                      'speed %.2f words/sec, time elapsed %.2f sec\n' % (epoch, train_iter,
                                                                       report_weighted_loss / report_examples,
                                                                       np.exp(report_loss / report_tgt_words),
                                                                       cum_examples,
                                                                       report_tgt_words / (time.time() - train_time),
            ...
            if train_iter % args.valid_niter == 0:
                print('epoch %d, iter %d, cum. loss %.2f, '
                      'cum. ppl %.2f cum. examples %d' % (epoch, train_iter,
                                                          ...
                                                          cum_examples),
                      file=sys.stderr)
                logs_raml.write('epoch %d, iter %d, cum. loss %.2f, '
                ...
                print('begin validation ...', file=sys.stderr)
                logs_raml.write('begin validation ...\n')
                model.eval()
                ...
                if args.valid_metric in ['bleu', 'word_acc', 'sent_acc']:
                    ...
                    train_iter, dev_ppl, args.valid_metric, valid_metric),
                          file=sys.stderr)
                    logs_raml.write('validation: iter %d, dev. ppl %f, dev. %s %f\n\n' % (
                    train_iter, dev_ppl, args.valid_metric, valid_metric))
                else:
                    valid_metric = -dev_ppl
                    print('validation: iter %d, dev. ppl %f' % (train_iter, dev_ppl),
                          file=sys.stderr)
                    logs_raml.write('validation: iter %d, dev. ppl %f\n\n' % (train_iter, dev_ppl))
                ...
                if valid_num > args.save_model_after:
                    model_file = args.save_to + '.iter%d.bin' % train_iter
                    print('save model to [%s]' % model_file, file=sys.stderr)
                    logs_raml.write('save model to [%s]\n\n' % model_file)
                    model.save(model_file)

                if (not is_better_than_last) and args.lr_decay:
                    lr = optimizer.param_groups[0]['lr'] * args.lr_decay
                    print('decay learning rate to %f' % lr, file=sys.stderr)
                    logs_raml.write('decay learning rate to %f\n\n' % lr)
                    optimizer.param_groups[0]['lr'] = lr
                ...
                if is_better:
                    ...
                    if valid_num > args.save_model_after:
                        print('save currently the best model ..', file=sys.stderr)
                        logs_raml.write('save currently the best model ..\n\n')
                        model_file_abs_path = os.path.abspath(model_file)
                        symlin_file_abs_path = os.path.abspath(args.save_to + '.bin')
                        os.system('ln -sf %s %s' % (model_file_abs_path, symlin_file_abs_path))
                else:
                    patience += 1
                    print('hit patience %d' % patience, file=sys.stderr)
                    logs_raml.write('hit patience %d\n\n' % patience)
                    if patience == args.patience:
                        print('early stop!', file=sys.stderr)
                        logs_raml.write('early stop!\n')
                        print('the best model is from iteration [%d]' % best_model_iter, file=sys.stderr)
                        logs_raml.write('the best model is from iteration [%d]\n\n' % best_model_iter)
                        exit(0)
...
def get_acc(references, hypotheses, acc_type='word'):
    ...
    for ref, hyp in list(zip(references, hypotheses)):
        ref = ref[1:-1]
        hyp = hyp[1:-1]
        if acc_type == 'word_acc':
            acc = len([1 for ref_w, hyp_w in list(zip(ref, hyp)) if ref_w == hyp_w]) / float(len(hyp) + 1e-6)
        else:
            acc = 1. if all(ref_w == hyp_w for ref_w, hyp_w in list(zip(ref, hyp))) else 0.
        cum_acc += acc
...
def compute_lm_prob(args):
    ...
    for src_sent, tgt_sent in test_data:
        ...
        for src_sent, tgt_sent, score in list(zip(src_sents, tgt_sents, tgt_sent_word_scores)):
    ...
    if args.save_to_file:
        ...
        if args.save_nbest:
            ...
            with open(nbest_file, 'w') as f:
                for src_sent, tgt_sent, hyps in list(zip(test_data_src, test_data_tgt, hypotheses)):
